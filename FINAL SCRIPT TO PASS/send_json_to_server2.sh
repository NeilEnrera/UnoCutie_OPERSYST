...
output_logfile="/opt/filesystem/send_json_to_server2_$(date '+%Y%m%d%H%M%S').log"  # Log file to capture script output

# Find the latest filesystem JSON file
json_filename=$(ls -t "$source_directory"/filesystem_*.json 2>/dev/null | head -1)  # Get the latest JSON file
...
remote_opt_directory="/opt/filesystem"  # Directory on the remote server to transfer the file
